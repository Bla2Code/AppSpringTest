 ...
 * P.S.: Привязать пользователя по внешнему ключу не представляется возможным,
 * поэтому в таблице сохраняется только имя пользователя (является уникальным) из таблицы users.
 */
@Getter
@Setter
@NoArgsConstructor
